...
class PdfReportGenerator {
      ...
      child: pw.Column(
        ...
        children: [
          ...
          if (imageUrls.isNotEmpty) ...[
            ...
            pw.Wrap(
              spacing: 8, // المسافة الأفقية بين الصور
              runSpacing: 8, // المسافة العمودية بين صفوف الصور
              alignment: pw.WrapAlignment.end, // محاذاة الصور إلى اليمين (بسبب RTL)
              children: imageUrls.map((imageUrl) {
                final image = fetchedImages[imageUrl];
                if (image != null) {
                  return pw.Container(
                    decoration: pw.BoxDecoration(
                      border: pw.Border.all(color: borderColor),
                    ...
                    // لا توجد أبعاد ثابتة هنا، ستظهر الصورة بحجمها الأصلي
                    child: pw.Image(image),
                  );
                }
                return pw.SizedBox.shrink(); // عنصر فارغ إذا لم يتم العثور على الصورة
              }).toList(),
            ),
          ],
        ],
            ...
            pw.Wrap(
              ...
              children: [
                pw.Container(
                  ...
                  // لا توجد أبعاد ثابتة هنا، ستظهر الصورة بحجمها الأصلي
                  child: pw.Image(fetchedImages[imgUrl]!),
                ),
              ],
            ),
          ],
  ...
  static Future<Uint8List> generateSimpleTables({ // Corrected: Uint8List
    required String projectId,
    required List<Map<String, dynamic>> phases,
    required List<Map<String, dynamic>> testsStructure,
    DateTime? start,
